...
// Eastmoney is the Eastmoney push2his daily-bar source (free, no auth). The
// endpoint rate-limits aggressively by source IP, so every request is spaced
// through the shared eastmoney throttle bucket.
...
const eastmoneyDefaultBaseURL = "https://push2his.eastmoney.com/api/qt/stock/kline/get"

// NewEastmoney returns an Eastmoney source. WithMinInterval(0) disables the
// per-request spacing.
func NewEastmoney(opts ...Option) *Eastmoney {
	o := defaultHTTPOptions(eastmoneyDefaultBaseURL)
	o.minInterval = time.Second // default 1s spacing (VIBE_TRADING_EASTMONEY_MIN_INTERVAL parity)
...
func (e *Eastmoney) Available(ctx context.Context) bool { return true }

// Daily fetches daily bars for an A-share symbol (SH -> secid 1.x, SZ/BJ -> 0.x).
func (e *Eastmoney) Daily(ctx context.Context, sym Symbol, start, end time.Time) ([]Bar, error) {
	var secid string
	switch sym.Exchange {
	case ExchangeSH:
		secid = "1." + sym.Code
	case ExchangeSZ, ExchangeBJ:
		secid = "0." + sym.Code
	default:
		return nil, fmt.Errorf("eastmoney: unsupported exchange %s", sym.Exchange)
	}

	if err := e.throttle.Wait(ctx, "eastmoney", e.minInterval); err != nil {
		return nil, err
	}
	...
	if err := json.Unmarshal(raw, &payload); err != nil {
		return nil, fmt.Errorf("eastmoney: decode body: %w", err)
	}
	if payload.Data == nil || len(payload.Data.Klines) == 0 {
		return nil, fmt.Errorf("eastmoney: empty payload for %s", sym)
	}

	bars := make([]Bar, 0, len(payload.Data.Klines))
	for _, rawRow := range payload.Data.Klines {
		// Column order: date, open, close, high, low, volume, amount.
	...
	if len(bars) == 0 {
		return nil, fmt.Errorf("eastmoney: no parseable bars for %s", sym)
	}
	return bars, nil
}
